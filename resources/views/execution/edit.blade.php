...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <!-- Fonts -->
        <link href="secure_asset{{'../../../../assets/css/review/edit.style.css'}}"  rel="stylesheet">
    </head>
    <body>
        @if($review)
        <div class="review">
            <form action="/reviews/{{$review->id}}/store" method="POST">
             @csrf 
             @method('PUT')
                <div class="date"><h2>{{$review->created_at->format('Y年m月d日')}}</h2></div>
                <div class="Review item">
                振り返り
                    <textarea type="comment" name="reviews[review]">{{$review->review }}</textarea>  
                </div>
                <div class="item">
                    <p>達成度</p>
        ...
        <div class="back">[<a href='/reviews/{{ $review->id }}'>Back</a>]</div>
        @endif
    </body>
</html>